"""Atari environment following the project's ``GymEnv`` protocol, backed by
...
(a ``(8,)`` uint8 array) is threaded through as the environment state. This wraps
it as a single-env (``num_envs == 1``), pure, ``jax.jit``/``lax.scan``-compatible
``reset``/``step`` matching :mod:`envs.gym_env`.
...
* **No auto-reset.** ale-py's XLA path only supports ``AutoresetMode.NEXT_STEP`` - on
  ...
  ``terminated | truncated``, and must do so *conditionally*: calling :meth:`reset` on a
  non-boundary step mutates the emulator, and no ``jnp.where`` can undo that.
* **Episode cutoff.** The *wrapper* owns truncation, not ale: it counts agent steps in
  ...
  ``environments.atari.build`` gives ale a limit one step *later*. So ale never sets
  ``done`` on a truncation, which is what would leave a pending autoreset mid-episode.
  ale's own ``truncations`` flag is OR-ed in as a backstop. (ale's limit is
  already exact in agent steps - it counts ``max_num_frames_per_episode /
  frameskip`` steps, excluding the random no-op start - so this buys the
  *ordering*, not accuracy.)
...
the in-step episode-boundary reset ran under a heavy graph like DQN. :meth:`step`
