"""DQN with an Equinox Q-network and Flashbax item buffer.

PureJaxRL-style scan loop: epsilon-greedy act, store transitions, TD update
from replay, Polyak target update. Bootstraps with ``(1 - terminated)`` only
(truncated episodes still bootstrap). On ``terminated | truncated``, resets
for the next step.
"""
...
def _polyak_update(target: QNetwork, online: QNetwork, tau: float) -> QNetwork:
...
def make_train(
    ...
    """Build the pure training function for a DQN agent.
    ...
    def train(rng: jax.Array) -> AgentTrainOutput:
        """Run one seed of DQN and return its runner state and metrics."""
        ...
        def _update_step(
            ...
            def _do_train(
                ...
                def loss_fn(q: QNetwork) -> jax.Array:
                    ...
                    next_q = jax.vmap(target_q)(batch.next_obs)
                    target = batch.reward + config.GAMMA * jnp.max(next_q, axis=-1) * (
